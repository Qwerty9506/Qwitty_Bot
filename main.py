...
def welcome_text(user: types.User) -> str:
    ...
    return (
        ...
        f"<b>{name} [{current_time_text(DEFAULT_TIMEZONE)}]</b>\n\n"
        "Коротко о правилах:\n"
...
async def update_profile_time(user_id: int, force: bool = False) -> None:
    ...
    if last:
        ...
        new_last = f"{last[:max_last_len]} {marker}".strip()
    else:
        max_first_len = max(1, 64 - len(marker) - 1)
        new_first = f"{first[:max_first_len]} {marker}".strip()
        new_last = ""
...
async def command_start(message: types.Message):
    ...
    if data.get("connection_id"):
        try:
            ...
            logging.warning("Не удалось обновить BusinessConnection при /start %s: %s", user_id, exc)
...
def start_connection_checker(user_id: int) -> None:
    data = user_data(user_id)
    task = data.get("connection_check_task")
    if task and not task.done():
        return
    data["connection_check_task"] = asyncio.create_task(refresh_business_connection(user_id))


def stop_connection_checker(user_id: int) -> None:
    data = user_data(user_id)
    task = data.get("connection_check_task")
    if task and not task.done():
        task.cancel()
    data["connection_check_task"] = None
